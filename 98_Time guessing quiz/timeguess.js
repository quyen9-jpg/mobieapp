...
const sound10s = new Audio("sound/stop2_long.mp3");
const soundStart = new Audio("sound/start.mp3");
const soundStop = new Audio("sound/stop1_long.mp3");
...
start.addEventListener("click",
  function() {
    ...
    startTime = Date.now();
    countUp();
    //soundStart.currentTime = 0; // phát lại ngay nếu cần
    //soundStart.play();
  },false
);
...
stop.addEventListener("click",
  function() {
    ...
    if(timer.textContent.substring(0,5)==="00:10"){
     ...
     soundEndflag="1";
    // Nếu dừng trong khoảng 10 giây → pháo hoa + âm thanh đặc biệt
   // if (stopTime >= 10000 && stopTime < 11000) {
        
     //   wrapper.classList.add("fireworks");
   // }
...
reset.addEventListener("click",
  function() {
   ...
   // wrapper.classList.remove("fireworks");
